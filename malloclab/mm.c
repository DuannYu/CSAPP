...
void *mm_malloc(size_t size)
{
    ...
    /* Adjust block size to include overhead and alignment steps */
    if (size <= DSIZE)
        asize = 2 * DSIZE;
    else 
...
static void *coalesce(void *bp)
{
    ...
    if (prev_alloc && next_alloc) {             /* Case 1: 不合并 */
        return bp;
    }

    else if (prev_alloc && !next_alloc) {       /* Case 2: 与后一个合并 */
        size += GET_SIZE(HDRP(NEXT_BLKP(bp)));
        PUT(HDRP(bp), PACK(size, 0));
        PUT(FTRP(bp), PACK(size, 0));
    }

    else if (!prev_alloc && next_alloc) {       /* Case 3: 与前一个合并 */
